...
export default function SignupPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      toast.error(locale === "ar" ? "كلمة المرور قصيرة جداً (6 على الأقل)" : "Password too short (min 6)");
      return;
    }
    setLoading(true);
    try {
      ...
      toast.success(
        locale === "ar"
          ? "تم إنشاء الحساب! تحقق من بريدك للتأكيد."
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-10 pb-24">
      <div className="text-center mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-1">{t.brand.tagline}</p>
      </div>

      <Card className="glass-card gold-glow">
        <CardHeader>
          <CardTitle className="text-base">{t.auth.signUp}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <p className="text-center text-sm text-muted-foreground">
            {t.auth.haveAccount}{" "}
            <Link href="/auth/login" className="text-gold hover:underline font-medium inline-flex items-center gap-1">
              {t.auth.signIn}
